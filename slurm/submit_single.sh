...
JOB_ID=$(
    AGENT_TYPE="$AGENT_TYPE" SEED="$SEED" TOTAL_EPS="$TOTAL_EPS" \
    sbatch \
        --job-name="himher_${AGENT_TYPE}_s${SEED}" \
        --export="AGENT_TYPE=${AGENT_TYPE},SEED=${SEED},TOTAL_EPS=${TOTAL_EPS}" \
        "$PROJECT_DIR/slurm/run_experiment.sh" \
    | awk '{print $NF}'
)

echo "Job submitted: ${JOB_ID}"
echo "Output: logs/slurm/himher_${AGENT_TYPE}_s${SEED}_${JOB_ID}.out"
